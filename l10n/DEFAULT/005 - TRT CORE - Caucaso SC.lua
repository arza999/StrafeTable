...
--↓↓ HEADQUARTERS ↓↓------------------------------------------------------------------------------------------------------------------------------------------------
local HQ = GROUP:FindByName("☮ 6th Air Division HQ")
local CommandCenterA2G = COMMANDCENTER:New( HQ, "6th Air Div" )
local CommandCenterHELI = COMMANDCENTER:New( HQ, "6th Tactical Div" )
local TRTPlanes = SET_GROUP:New():FilterCoalitions( "blue" ):FilterPrefixes( "✈" ):FilterStart()
local TRTHelis = SET_GROUP:New():FilterCoalitions( "blue" ):FilterPrefixes( "✈" ):FilterCategoryHelicopter():FilterStart()
...
local tanker=RECOVERYTANKER:New(UNIT:FindByName("☮ CVN-73 George Washington"), "☮ YPF")  
:SetTakeoffAir()
:SetAltitude(Tanker_BOOM_Altitude)
:SetSpeed(Tanker_BOOM_Speed)
:SetHomeBase(AIRBASE.Caucasus.Kutaisi)
:SetRadio(255)
:SetModex(511)
:SetTACAN(24, "YPF")
:SetRacetrackDistances(50, 5)
:Start()

local tanker2=RECOVERYTANKER:New(UNIT:FindByName("☮ CVN-73 George Washington"), "☮ ARCO")  
:SetTakeoffAir()
...
local awacs=RECOVERYTANKER:New(UNIT:FindByName("☮ CVN-73 George Washington"), "☮ Wizard") 
:SetAWACS()
:SetTakeoffAir()
:SetRadio(251)
:SetAltitude(20000)
:SetHomeBase("☮ CVN-73 George Washington")
:SetCallsign(CALLSIGN.AWACS.Wizard)
:SetRacetrackDistances(40, 5)
:SetTACAN(26, "WIZ")
:SetModex(611)
:__Start(1)

if enable_RescueHelos == true then
local rescuehelo=RESCUEHELO:New(UNIT:FindByName("☮ CVN-73 George Washington"), "☮ Rescate")  
:SetHomeBase(AIRBASE:FindByName("Tico_A"))
:SetTakeoffAir()
:SetRespawnInAir()
:SetRescueDuration(1)
:SetModex(42)
...
local rescueheloB=RESCUEHELO:New(UNIT:FindByName("☮ LHA-1 Tarawa"), "☮ Rescate")
:SetHomeBase(AIRBASE:FindByName("Tico_B"))
:SetTakeoffAir()
:SetRespawnInAir()
:SetRescueDuration(1)
:SetModex(43)
...
if enable_Airboss == true then
local AirbossStennis=AIRBOSS:New("☮ CVN-73 George Washington")
local window1=AirbossStennis:AddRecoveryWindow( "08:59", "14:55", 1, nil, false)
local window2=AirbossStennis:AddRecoveryWindow("15:00", "20:55", 2, nil, false)
local window3=AirbossStennis:AddRecoveryWindow("21:00", "08:00+1", 3, nil, false)
...
AirbossStennis:SetRadioRelayLSO("Stennis LSO")
AirbossStennis:SetRadioRelayMarshal("Stennis Marshal")
AirbossStennis:SetHandleAIOFF()
AirbossStennis:Start()

local AirbossTarawa=AIRBOSS:New("☮ LHA-1 Tarawa")
local window1=AirbossTarawa:AddRecoveryWindow( "08:59", "14:55", 1, nil, false)
local window2=AirbossTarawa:AddRecoveryWindow("15:00", "20:55", 2, nil, false)
local window3=AirbossTarawa:AddRecoveryWindow("21:00", "06:00+1", 3, nil, false)
AirbossTarawa:SetTACAN(75, "X", "LHA")
AirbossTarawa:Load()
AirbossTarawa:SetAutoSave()
AirbossTarawa:SetHandleAIOFF()
AirbossTarawa:SetRadioRelayLSO("Tarawa LSO")
AirbossTarawa:SetRadioRelayMarshal("Tarawa Marshal")
...
if enable_Airboss == false then
  ...
  BARCO03 = GROUP:FindByName("☮ CVN-73 George Washington"):PatrolRoute()
  local StennisTCNU = UNIT:FindByName("☮ CVN-73 George Washington") 
  local StennisBeacon = StennisTCNU:GetBeacon():ActivateTACAN(74, "X", "STN", true):ActivateICLS(1, "STN")
end 

--trigger.action.outText("➫ AIRBOSS: ✓✓", 10)
--FIN---------------------------------------------------------------------------------------------------------------------------------------------------------------
--↓↓ A2A ↓↓---------------------------------------------------------------------------------------------------------------------------------------------------------
--BlueCapPlanesLIST = { "☮ F18", "☮ M2000", "☮ F14", "☮ F15", "☮ F16" }
--RedCapPlanesLIST = { "☢ MiG-31", "☢ MiG-29S", "☢ MiG-21Bis", "☢ Su-27", "☢ Su-33", "☢ MiG-19P", "☢ MiG-23MLD", "☢ MiG-25PD", "☢ MiG-27K", "☢ Su-17M4", "☢ Su-24M", "☢ Su-34", "☢ F5E3", "☢ F-15C", "☢ F-18C", "☢ F-14B", "☢ JF-17", "☢ J-11", "☢ Mirage 2000" }

ZONA_REDA2A_SPAWN01 = ZONE:New('ZONA_REDA2A_SPAWN01')
ZONA_REDA2A_SPAWN02 = ZONE:New('ZONA_REDA2A_SPAWN02')
ZONA_REDA2A_SPAWN03 = ZONE:New('ZONA_REDA2A_SPAWN03')
RedCAPzonelist = {ZONA_REDA2A_SPAWN01, ZONA_REDA2A_SPAWN02, ZONA_REDA2A_SPAWN03}

ZONA_BLUEA2A_SPAWN01 = ZONE:New('ZONA_BLUEA2A_SPAWN01')
ZONA_BLUEA2A_SPAWN02 = ZONE:New('ZONA_BLUEA2A_SPAWN02')
ZONA_BLUEA2A_SPAWN03 = ZONE:New('ZONA_BLUEA2A_SPAWN03')
BlueCAPzonelist = {ZONA_BLUEA2A_SPAWN01, ZONA_BLUEA2A_SPAWN02, ZONA_BLUEA2A_SPAWN03}

local RNGRedCAPZone = math.random(1, #(RedCAPzonelist))
local RNGBlueCAPZone = math.random(1, #(BlueCAPzonelist))
...
if Enable_CAT_Squadron == true then
  CATSQDN_CAP =
      ...
      1,
      20000
  ):InitCleanUp(60):InitLimit(TotalAliveRedPlanes, 0)

  function CATSQDN_CAP_start()
      CATSQDN_CAP:SpawnInZone(RedCAPzonelist[RNGRedCAPZone], true, 5000, 10000)
  end
  ...
  timer.scheduleFunction(
      ...
      nil,
      timer.getTime() + math.random(120, 900)
  )
end
...
if Enable_BEAR_Squadron == true then
  ...
  function BEARSQDN_CAP_Timer_start()
      BEARSQDN_CAP_Timer:SpawnInZone(RedCAPzonelist[RNGRedCAPZone], true, 5000, 10000)
  end
...
if Enable_TIGER_Squadron == true then
  ...
  function TIGERSQDN_CAP_Timer_start()
      TIGERSQDN_CAP_Timer:SpawnInZone(RedCAPzonelist[RNGRedCAPZone], true, 5000, 10000)
  end
...
if Enable_FALCON_Squadron == true then
  FALCONSQDN_CAP_Timer =
      ...
      1,
      20000
  ):InitCleanUp(60):InitLimit(TotalAliveBluePlanes, 0)

  function FALCONSQDN_CAP_Timer_start()
      FALCONSQDN_CAP_Timer:SpawnInZone(BlueCAPzonelist[RNGBlueCAPZone], true, 5000, 10000)
  end
...
if Enable_HAWK_Squadron == true then
  ...
  function HAWKSQDN_CAP_Timer_start()
      HAWKSQDN_CAP_Timer:SpawnInZone(BlueCAPzonelist[RNGBlueCAPZone], true, 5000, 10000)
  end
...
if Enable_EAGLE_Squadron == true then
  ...
  function EAGLESQDN_CAP_Timer_start()
      EAGLESQDN_CAP_Timer:SpawnInZone(BlueCAPzonelist[RNGBlueCAPZone], true, 5000, 10000)
  end
...
if enable_MissileTrainer == true then
  fox = FOX:New()
  fox:AddSafeZone(ZONE:New('Kutaisi'))
  fox:AddSafeZone(ZONE:New('Senaki'))
  fox:AddSafeZone(ZONE:New('Sujumi'))
...
TempBAJASAA = {BajasAA1, BajasAA2, BajasAA3, BajasAA4, BajasAA5, BajasAA6,BajasAA7,BajasAA8,BajasAA9,BajasAA10,BajasAA11,BajasAA12}
...
TempBAJASAG = {BajasAG1, BajasAG2, BajasAG3, BajasAG4, BajasAG5, BajasAG6,BajasAG7,BajasAG8}
...
function Contadorde_BajasAA:OnEventCrash(EventData)
	...
	if ( CrashedUnitCoalition == 1 ) then
		--trigger.action.outText("✗ A " .. CrashedUnitType .. " has been shot down!",15)
		SNDBAJAEN = SonidoBajaAA()
		SNDBAJAEN:ToCoalition( coalition.side.BLUE )			
	elseif ( CrashedUnitCoalition == 2 ) then 
		--trigger.action.outText("✝ An allied " .. CrashedUnitType .. " has been shot down!",15)
...
if ( SFX_Misc == 1 ) then
  ...
  SET_Aterrizajes = SET_UNIT:New():FilterPrefixes( { "✈" } ):FilterCoalitions( { "Blue" } ):FilterStart()
  SET_Aterrizajes:HandleEvent(EVENTS.Land)


  function SET_Aterrizajes:OnEventLand(EventData)

	local GRUPODEIAOJUGADOR = EventData.IniDCSGroupName
	local NOMBREDEUNIDAD = EventData.IniTypeName
	local LUGARDEATERRIZAJE = EventData.PlaceName
	local NOMBREDEJUGADOR = EventData.IniPlayerName
  
	if string.find(GRUPODEIAOJUGADOR, "✈") then
		trigger.action.outText(NOMBREDEJUGADOR.. " landed his " .. NOMBREDEUNIDAD .. " at " .. LUGARDEATERRIZAJE,15)
...
TRTOpcionesSoporteF16CAS = missionCommands.addCommandForCoalition(coalition.side.BLUE, "CAS F-16", TRTOpcionesSoporte, function() CASF16() end, nil)
TRTOpcionesSoporteF18SEAD = missionCommands.addCommandForCoalition(coalition.side.BLUE, "SEAD F-18", TRTOpcionesSoporte, function() SEADF18() end, nil)
TRTOpcionesSoporteF16SEAD = missionCommands.addCommandForCoalition(coalition.side.BLUE, "SEAD F-16", TRTOpcionesSoporte, function() SEADF16() end, nil)
TRTOpcionesSoporteF16SEAD = missionCommands.addCommandForCoalition(coalition.side.BLUE, "ANTI-SHIP Su-34", TRTOpcionesSoporte, function() ANTISHIPSU34() end, nil)
TRTOpcionesSoporteF16SEAD = missionCommands.addCommandForCoalition(coalition.side.BLUE, "ANTI-SHIP Viggen", TRTOpcionesSoporte, function() ANTISHIPViggen() end, nil)
end
...
TRTOpcionesMakeBAI = missionCommands.addCommandForCoalition(coalition.side.BLUE, "Create BAI Task", TRTOpcionesTASKSMAIN, function() MakeTaskBAI_menucmd() end, nil)
TRTOpcionesMakeBAIHard = missionCommands.addCommandForCoalition(coalition.side.BLUE, "Create BAI (Hard) Task", TRTOpcionesTASKSMAIN, function() MakeTaskBAIHARD_menucmd() end, nil)
TRTOpcionesMakeSEAD = missionCommands.addCommandForCoalition(coalition.side.BLUE, "Create SEAD Task", TRTOpcionesTASKSMAIN, function() MakeTaskSEAD_menucmd() end, nil)
TRTOpcionesMakeAship = missionCommands.addCommandForCoalition(coalition.side.BLUE, "Create Anti Ship Task", TRTOpcionesTASKSMAIN, function() MakeTaskNavBAI_menucmd() end, nil)
end
...
function DesactivarSFXMisc()
  if (SFX_Misc == 0) then
    SFX_Misc = 1
      trigger.action.outText('Misc SFX are now enabled', 15)
  elseif (SFX_Misc == 1) then
    SFX_Misc = 0
      trigger.action.outText('Misc SFX are now Disabled', 15)
   else
  end
end
...
function CurrentMissionSettings()
  ...
  if (enable_Airboss == true) then
      trigger.action.outText('AIRBOSS: ✔', 20)
  end
  if (enable_RescueHelos == true) then
      trigger.action.outText('AIRBOSS Rescue Helos: ✔', 20)
  end
  if (allow_EndMissionViaRadio == true) then
  ...
  if (allow_ManualTaskCreation == true) then
      trigger.action.outText('You can create A2G task manually via radio', 20)
  end
end
...
function ANTISHIPSU34()
  ANTISHIPSU34 = SPAWN:New('☮ Su-34 AntiShip'):InitCleanUp(60):InitLimit(2, 0):Spawn()
end

function ANTISHIPViggen()

...
function findNearestVisibleEnemy(jtacUnit, targetType,distance)
...
-- function findNearestVisibleEnemy(jtacUnit, targetType)
	...
	-- finish this function
    local vhpriority = false
    local vpriority = false
    local thpriority = false
    local tpriority = false
    for i = 1, #redGroups do
        if redGroups[i] ~= nil then
            groupName = redGroups[i]:getName()
            units = getGroup(groupName)
            if #units > 0 then
                for y = 1, #units do
                    ...
                    if tempDist < JTAC_maxDistance and tempDist < nearestDistance then
                        ...
                        if land.isVisible(offsetEnemyPos, offsetJTACPos) and targeted == false then
                            if (string.match(units[y]:getName(), "hpriority") ~= nil) and isVehicle(units[y]) then
                                vhpriority = true
                            elseif (string.match(units[y]:getName(), "priority") ~= nil) and isVehicle(units[y]) then
                                vpriority = true
                            elseif (string.match(units[y]:getName(), "hpriority") ~= nil) and isInfantry(units[y]) then
                                thpriority = true
                            elseif (string.match(units[y]:getName(), "priority") ~= nil) and isInfantry(units[y]) then
                                tpriority = true
                            end
                        end
                    end
        ...
        if redGroups[i] ~= nil then
            ...
            if #units > 0 then
                ...
                for x = 1, #units do
                    ...
                    if targetType == "vehicle" and vhpriority == true then
                        allowedTarget = (string.match(units[x]:getName(), "hpriority") ~= nil) and isVehicle(units[x])
                    elseif targetType == "vehicle" and vpriority == true then
                        allowedTarget = (string.match(units[x]:getName(), "priority") ~= nil) and isVehicle(units[x])
                    elseif targetType == "vehicle" then
                        allowedTarget = isVehicle(units[x])
                    elseif targetType == "troop" and thpriority == true then
                        allowedTarget = (string.match(units[x]:getName(), "hpriority") ~= nil) and isInfantry(units[x])
                    elseif targetType == "troop" and tpriority == true then
                        allowedTarget = (string.match(units[x]:getName(), "priority") ~= nil) and isInfantry(units[x])
                    elseif targetType == "troop" then
                        allowedTarget = isInfantry(units[x])
                    elseif vhpriority == true or thpriority == true then
                        allowedTarget = (string.match(units[x]:getName(), "hpriority") ~= nil)
                    elseif vpriority == true or tpriority == true then
                        allowedTarget = (string.match(units[x]:getName(), "priority") ~= nil)
                    else
                        allowedTarget = true
                    end
...
-- gets the JTAC status and displays to coalition units
function getJTACStatus(groupId)
